...
class FilterController extends Controller
{
    ...
    function getFilterBuktiKontrolMutasi(){
        try {
            ...
            $sql="select distinct no_bukti, keterangan from trans_m where kode_lokasi='$kode_lokasi'";
    ...
    function getFilterBuktiMutasiTerima(){
        try {
            ...
            $sql="select distinct no_bukti, keterangan from trans_m where kode_lokasi='$kode_lokasi' and no_bukti like 'MT%' ";
            $res = DB::connection($this->db)->select($sql);
            $res = json_decode(json_encode($res),true);
    ...
    function getFilterBuktiMutasiKirim(){
        try {
    ...
    function getFilterBuktiMutasi(Request $request){
        try {
        ...
        try {
            ...
            if(isset($request->jenis_akun)){
                if($request->jenis_akun == "KAS"){
                    ...
                    where a.block= '0' and b.kode_flag in ('001','009') and a.kode_curr='IDR' and a.kode_lokasi='$kode_lokasi' ";
                }else{
    ...
    function getFilterPeriodeKB(){
        try {
            ...
            $sql="select distinct a.periode,dbo.fnNamaBulan(a.periode) as nama from trans_m a where a.periode <> '' and a.kode_lokasi='$kode_lokasi' and a.modul in ('KB','KBSPB','KBSPBPJ') ";
            $res = DB::connection($this->db)->select($sql);
            $res = json_decode(json_encode($res),true);
    ...
    function getFilterBuktiJurnalKB(Request $request){
        try {
            ...
            $sql="select a.no_bukti,a.keterangan 
            from trans_m a 
            $where and a.modul in ('KB','KBSPB','KBSPBPJ')";
            $res = DB::connection($this->db)->select($sql);
            $res = json_decode(json_encode($res),true);
            if(count($res) > 0){ //mengecek apakah data kosong atau tidak
                $success['status'] = true;
    ...
    function getFilterTahun(){
        try {
            ...
            $sql="select distinct substring(periode,1,4) as periode from periode where kode_lokasi='".$kode_lokasi."' order by substring(periode,1,4) desc";
            $res = DB::connection($this->db)->select($sql);
            $res = json_decode(json_encode($res),true);
