...
class PembelianPPnController extends Controller
{
    ...
    //                 $harga = floatval($request->harga_barang[$a])*(100/111); //nilai dpp
    //             } else {
    ...
    //                 $request->harga_jual[$a],   //nampung aja utk storproc
    //                 $request->harga_barang[$a], //nampung aja utk storproc
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $total_subhit = floatval($request->total_trans);
            ...
            for($a=0; $a<count($request->kode_barang);$a++){
                ...
                $sub = floatval($request->sub_barang[$a]);
                $sub_hitung = floatval($request->subhit_barang[$a]);
            ...
            // Jika di kelompokan berdasarkan kode_akun, nilai & nilai_curr di isi $series2[$series[$x]]
            // Jika nilainya berdasarkan yang nilai yang di tampilkan, nilai & nilai_curr di isi $total_subhit
            ...
            DB::connection($this->sql)->insert($insertM, [
                ...
                1,
                round(floatval($total_subhit),0), 
                round(floatval($totPPN),0), // nilai2 (asalnya total PPn)
                round($totDiskon,0),
                '-',
                '-',
                '-',
            ...
            DB::connection($this->sql)->insert($insertB, [
               ...
               1,
               $request->kode_pp,
               round(floatval($total_subhit),0), // Nilai Sub Total Hitung
               $periode,
               $nik,
               date('Y-m-d H:i:s'),
               $akunHutang,
               round($totPPN,0), // Total PPn
               '-',
               date('Y-m-d H:i:s'),
               0,
               round($totDiskon,0),
            ...
            if ($nilai_trans != 0) {
                ...
                Log::info('Nilai Trans '.$nilai_trans);
            }
