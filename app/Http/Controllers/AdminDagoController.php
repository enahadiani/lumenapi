...
class AdminDagoController extends Controller
{
    ...
    public function profile()
    {
        if($data =  Auth::guard('dago')->user()){
            ...
            if(count($user) > 0){ //mengecek apakah data kosong atau tidak
                $periode = DB::connection('sqlsrvdago')->select("select max(periode) as periode from periode where kode_lokasi='$kode_lokasi'
                ");
                $periode = json_decode(json_encode($periode),true);

                $fs = DB::connection('sqlsrvdago')->select("select kode_fs from fs where kode_lokasi='$kode_lokasi'
                ");
    ...
    public function profileMobileApv()
    {
        if($data =  Auth::guard('dago')->user()){
            ...
            $user = DB::connection('sqlsrvdago')->select("select a.nik, c.nama,
    ...
    public function updatePassword(Request $request){
        ...
        try {
            ...
            $upd =  DB::connection('sqlsrvdago')->table('hakakses')
            ->where('nik', $nik)
    ...
    {
         return response()->json(['users' =>  dago::all()], 200);
    }
    ...
    public function cekPayload(){
        $payload = Auth::guard('dago')->payload();
        // $payload->toArray();
