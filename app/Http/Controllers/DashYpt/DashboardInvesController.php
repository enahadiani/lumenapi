...
class DashboardInvesController extends Controller {
    ...
    public function getDataBox(Request $r) {
        try {
            ...
            case when sum(a.n2) <> 0 then (sum(a.n6)/sum(a.n2))*100 else 0 end as capai_ytd,
            case when sum(a.n1) <> 0 then (sum(a.n6)/sum(a.n1))*100 else 0 end as capai_tahun,
            case when sum(a.n7) <> 0 then ((sum(a.n6)-sum(a.n7))/sum(a.n7))*100 else 0 end as yoy
            from exs_neraca a
    ...
    public function getSerapAgg(Request $r) {
        try {
            ...
            case when sum(a.n1)<>0 then (sum(a.n6)/sum(a.n1))*100 else 0 end as ach
            from exs_neraca a
            inner join dash_ypt_neraca_d b on a.kode_neraca=b.kode_neraca and a.kode_fs=b.kode_fs
