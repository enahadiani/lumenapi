...
class FilterController extends Controller
{
    ...
    function getFilterPeriode(Request $request){
        try {
            ...
            $sql="select distinct periode,nama from (select a.periode,dbo.fnNamaBulan(a.periode) as nama from gr_pb_m a where a.kode_lokasi='$kode_lokasi'
            union all
            select a.periode,dbo.fnNamaBulan(a.periode) as nama from gr_spb2_m a where a.kode_lokasi='$kode_lokasi'
            ) a 
            ";
            $res = DB::connection($this->db)->select($sql);
            $res = json_decode(json_encode($res),true);
    ...
    function getFilterNoBukti(Request $request){
        try {
            ...
            $col_array = array('periode','kode_pp');
            $db_col_name = array('a.periode','a.kode_pp');
    ...
    function getFilterNoBuktiSPB(Request $request){
        try {
            ...
            // $success['where'] = $where;
            $sql="select a.spb,a.keterangan from gr_spb2_m a $where ";
            $res = DB::connection($this->db)->select($sql);
            $res = json_decode(json_encode($res),true);
    ...
    function getFilterPeriodeKeuangan(){
        try {
            ...
            $sql="select distinct a.periode from periode a ";
            $res = DB::connection($this->db)->select($sql);
            $res = json_decode(json_encode($res),true);
