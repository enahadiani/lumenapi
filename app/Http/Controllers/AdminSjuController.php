...
class AdminSjuController extends Controller
{
     /**
    ...
    public function profile()
    {
        if($data =  Auth::guard('sju')->user()){
			...
			b.kode_lokkonsol,d.kode_bidang, case when c.foto != '-' then '".$url."/'+c.foto else '-' end as foto,isnull(e.form,'-') as path_view,b.logo,c.no_telp,c.jabatan,isnull(c.id_device,'-') as id_device
            from hakakses a 
            ...
            if(count($user) > 0){ //mengecek apakah data kosong atau tidak
                $periode = DB::connection('sqlsrvsju')->select("select max(periode) as periode from periode where kode_lokasi='$kode_lokasi'
                ");
                $periode = json_decode(json_encode($periode),true);

                $fs = DB::connection('sqlsrvsju')->select("select kode_fs from fs where kode_lokasi='$kode_lokasi'
                ");
    ...
    {
         return response()->json(['users' =>  AdminSju::all()], 200);
    }
    ...
    public function cekPayload(){
        $payload = Auth::guard('sju')->payload();
        // $payload->toArray();
