...
class AdminRtrwController extends Controller
{
     /**
    ...
    public function profile()
    {
        if($data =  Auth::guard('rtrw')->user()){
			...
			b.kode_lokkonsol,d.kode_bidang, c.foto,isnull(e.form,'-') as path_view,b.logo,c.no_telp,c.jabatan,a.kode_rumah,a.kode_klp_menu as kode_menu,b.rw
            from hakakses a 
            ...
            if(count($user) > 0){ //mengecek apakah data kosong atau tidak
                $periode = DB::connection('sqlsrvrtrw')->select("select max(periode) as periode from periode where kode_lokasi='$kode_lokasi'
                ");
                $periode = json_decode(json_encode($periode),true);

                $fs = DB::connection('sqlsrvrtrw')->select("select kode_fs from fs where kode_lokasi='$kode_lokasi'
                ");
    ...
    {
         return response()->json(['users' =>  AdminRtrw::all()], 200);
    }
