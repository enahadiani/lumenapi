...
class AdminItplnController extends Controller
{
    ...
    public function profile()
    {
        if($data =  Auth::guard($this->guard)->user()){
			...
			b.kode_lokkonsol,d.kode_bidang, c.foto,isnull(e.form,'-') as path_view,b.logo,c.no_telp,c.jabatan, isnull(a.flag_menu,'-') as flag_menu
            from hakakses a 
    ...
    {
         return response()->json(['users' =>  AdminItpln::all()], 200);
    }
        ...
        try {
            ...
            if($request->hasfile('foto')){
                ...
                if(count($res) > 0){
                    ...
                    if($foto != ""){
                        Storage::disk('s3')->delete('itpln/'.$foto);
                    }
                }else{
                    $foto = "-";
                }
                ...
                $foto = $nama_foto;
                if(Storage::disk('s3')->exists('itpln/'.$foto)){
                    Storage::disk('s3')->delete('itpln/'.$foto);
                }
                Storage::disk('s3')->put('itpln/'.$foto,file_get_contents($file));
    ...
    public function getDaftarPP(Request $request)
    {
        ...
        try {
            ...
            $res = DB::connection('dbpln')->select("SELECT a.kode_pp,a.nama from pp a left join sis_hakakses b on a.kode_pp = b.kode_pp where a.kode_lokasi = '01' and b.nik='$request->nik' order by a.kode_pp
            ");
            $res = json_decode(json_encode($res),true);
