...
class AnperController extends Controller {
    ...
    function getNeraca(Request $request,$anper){
        try {
            ...
            $dbh = DB::connection($this->db)->getPdo();
            $sth = $dbh->prepare("SET NOCOUNT ON; EXEC dbtm1.dbo.sp_neraca_dw '$kode_fs','N','S','$level','$periode','$kode_lokasi','$nik_user'; ");
            $sth->execute();
            ...
            if($format == "Mutasi"){
                ...
                case jenis_akun when  'Pendapatan' then -(n2-n3) else n2-n3 end as n4
                from dbtm1.dbo.neraca_tmp 
                where modul in ('A','P') and nik_user='$nik_user' 
