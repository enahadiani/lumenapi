...
{    
    ...
    public function index(Request $request)
    {
        try {            
            ...
            $sql= "select no_bukti,keterangan,periode,total_upload,case when datediff(minute,tgl_input,getdate()) <= 10 then 'baru' else 'lama' end as status,tgl_input from yks_arus_kas_m where kode_lokasi='".$kode_lokasi."' ";
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $per = date('ym');
            $no_bukti = $this->generateKode("yks_arus_kas_m", "no_bukti", $kode_lokasi."-UAK".$per.".", "0001");

            $insm = DB::connection($this->sql)->insert("insert into yks_arus_kas_m(no_bukti,kode_lokasi,periode,keterangan,total_upload,tgl_input,nik_user) select  '$no_bukti','$kode_lokasi','$request->periode','$request->keterangan', count(jenis),getdate(),'$nik' from yks_arus_kas_tmp where nik_user='$request->nik_user' and periode ='$request->periode' ");
            ...
            $insd = DB::connection($this->sql)->insert("insert into yks_arus_kas_d(no_bukti,no_urut,periode,keterangan,n1,n2,jenis,tgl_input,nik_user) 
                select '$no_bukti',no_urut,periode,keterangan,n1,n2,jenis,tgl_input,'$nik' as nik_user from yks_arus_kas_tmp where nik_user='$request->nik_user' and periode ='$request->periode'  ");
            ...
            $success['status'] = false;
            $success['message'] = "Data Arus Kas gagal disimpan ".$e;
            return response()->json($success, $this->successStatus); 
        }				
    ...
    public function importExcel(Request $request)
    {
        ...
        try {
            ...
            $del1 = DB::connection($this->sql)->table('yks_arus_kas_tmp')->where('periode', $request->periode)->where('nik_user', $request->nik_user)->delete();
            ...
            $dt = Excel::toArray(new ArusKasImport(),$nama_file);
            $excel = $dt[0];
            $x = array();
            $query = "";
    ...
    public function export(Request $request) 
    {
        ...
        if(isset($request->type) && $request->type == "template"){
            return Excel::download(new ArusKasExport($request->nik_user,$request->periode,$request->type), 'ArusKas_'.$request->nik_user.'.xlsx');
        }else{
            return Excel::download(new ArusKasExport($request->nik_user,$request->periode,$request->type), 'ArusKas_'.$request->nik_user.'.xlsx');
        }
    }
