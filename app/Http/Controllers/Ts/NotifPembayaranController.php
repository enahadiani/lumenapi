...
class NotifPembayaranController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            from sis_pesan_m a
            where a.kode_lokasi='$kode_lokasi' and a.nik_user='$nik' and a.jenis='Pembayaran' $filter");
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            from sis_hakakses a
            inner join ( select a.no_rekon,a.nis,a.kode_pp,a.kode_lokasi,sum(a.nilai) as nilai
                         from sis_rekon_d a 
            ...
            $key = array(
                ...
                'snap_token' => '-',
                'status' => 'pembayaran',
                'no_rekon' => $request->no_rekon,
                'nik' => $nik
            );
            
            $ins = DB::connection($this->db)->insert("insert into sis_pesan_m(no_bukti,jenis,nis,kode_akt,kode_kelas,judul,subjudul,pesan,kode_pp,kode_lokasi,ref1,ref2,ref3,link,tipe,tgl_input,nik_user,kode_matpel) values ('$no_bukti','Pembayaran','-','-','-','$request->judul','-','-','$kode_pp','$kode_lokasi','-','-','-','-','info',getdate(),'$nik','-') ");
            ...
            if(count($ck) > 0){
                for($i=0;$i<count($ck);$i++){
                    if(!isset($nik[$ck[$i]['nik']])){
                        $ins2[$i] = DB::connection($this->db)->insert("insert into sis_pesan_d(no_bukti,kode_lokasi,kode_pp,sts_read,sts_read_mob,id_device,nik,pesan,ref1,ref2,ref3) values ('$no_bukti','$kode_lokasi','$kode_pp','0','0','".$ck[$i]['id_device']."','".$ck[$i]['nik']."','Pembayaran anda sebesar ".number_format($ck[$i]['nilai'],0,",",".")."','".$request->no_rekon."','".$request->periode."','-') ");
                    }
            ...
            $jml = count($arr_id);
            Log::info('Jumlah Notif Pembayaran:'.$jml);
            return response()->json($success, $this->successStatus);     
        } catch (\Throwable $e) {
            DB::connection($this->db)->rollback();
            Log::error('Error Notif Pembayaran:'.$e);
            $success['status'] = false;
    ...
    public function getPeriode(Request $request){
        ...
        try{
			...
			$sql = "select distinct periode as periode from sis_rekon_d where kode_lokasi='$kode_lokasi' and kode_pp='$kode_pp'
    ...
    public function getNoRekon(Request $request){
        ...
        try{
			...
			$sql = "select no_rekon,keterangan from sis_rekon_m where kode_lokasi='$kode_lokasi' and kode_pp='$kode_pp' $filter
			";
			$res = DB::connection($this->db)->select($sql);
			$res = json_decode(json_encode($res),true);
			if(count($res) > 0){ //mengecek apakah data kosong atau tidak
