                            ...
                            if(floatval($total_bruto) != floatval($total_biaya)){
                                ...
                                if (floatval($total_pajak) != 0 && $datam['status_pajak'] == "NON") {
                                    ...
                                    if (floatval($total_bruto) > floatval($saldo_or)) {
                                        ...
                                        $ins4 = DB::connection($this->db)->insert("insert into prb_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,nik_user,kode_proyek,form) values ('".$no_bukti."','".$kode_lokasi."','".$periode."','".$periode."','".$datam['tanggal']."','".$akun_beban."','".$ppKelola."','-','".$datam['keterangan']."','D',".$nilaiBeban.",getdate(),'$nik','".$datam['kode_proyek']."','NTF19')");	
