...
{
    if (!Storage::disk('s3')->exists('dash-ypt/'.$filename)) {
        abort(404);
    }
    return Storage::disk('s3')->response('dash-ypt/'.$filename); 
});
...
$router->group(['middleware' => 'auth:yptkug'], function () use ($router) {
    ...
    //Menu
    $router->get('menu/{kode_klp}', 'DashYpt\MenuController@show');
    
    $router->post('notif-pusher', 'DashYpt\NotifController@sendPusher');
    $router->get('notif-pusher', 'DashYpt\NotifController@getNotifPusher');
    $router->put('notif-update-status', 'DashYpt\NotifController@updateStatusRead');
